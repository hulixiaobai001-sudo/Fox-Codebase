...
function hG(){return typeof G!=='undefined'&&G&&typeof G.p!=='undefined'&&G.p.length===2}
function iG(){return hG()&&G.phase!=='setup'&&G.phase!=='go'}

// Save
function cS(){if(!hG())return null;return{v:3,t:Date.now(),mode:G.mode,phase:G.phase,mult:G.mult,rd:G.rd,tr:G.tr,cp:G.cp,ld:G.ld,bp:G.bp,cc:G.cc,br:G.br,rs:G.rs,bf:G.bf,de:G.de,da:G.da,st:G.st,ps:G.ps,certShown:G.certShown,s:G.s,p:JSON.parse(JSON.stringify(G.p)),log:(G.log||[]).slice(-30)}}
function rS(s){if(!hG())return false;G.mode=s.mode||'local';G.phase=s.phase||'setup';G.mult=s.mult||1;G.rd=s.rd||0;G.tr=s.tr||0;G.cp=s.cp||0;G.ld=s.ld||0;G.bp=s.bp!=null?s.bp:-1;G.cc=s.cc||0;G.br=!!s.br;G.rs=!!s.rs;G.bf=!!s.bf;G.de=!!s.de;G.da=!!s.da;G.st=s.st||0;G.ps=s.ps||0;G.certShown=!!s.certShown;
  if(s.s){G.s.hp=s.s.hp||5;G.s.ch=s.s.ch||6;G.s.bu=s.s.bu||1;G.s.sk=s.s.sk||1;G.s.it=s.s.it||4;G.s.noSk=!!s.s.noSk}
  if(s.p&&s.p.length===2){for(var i=0;i<2;i++){var src=s.p[i],dst=G.p[i];dst.n=src.n||'';dst.hp=src.hp!=null?src.hp:5;dst.mx=src.mx||5;dst.al=src.al!==false;dst.ai=!!src.ai;dst.it=JSON.parse(JSON.stringify(src.it||{}));if(src.sel)dst.sel=src.sel.slice();dst.wa=!!src.wa;dst.wp=src.wp||0;dst.ba=!!src.ba;dst.bu=!!src.bu;dst.fa=!!src.fa;dst.fc=src.fc||0;dst.ft=src.ft||0}}
  if(s.log)G.log=s.log.slice();uUI();return true}
function uUI(){if(!hG())return;var p=G.phase,su=$('scrSetup'),gb=$('gameBoard');if(p==='setup'||p==='go'||p==='gameover'){if(su)su.classList.add('on');if(gb)gb.classList.remove('on');G.phase='setup';return}if(su)su.classList.remove('on');if(gb)gb.classList.add('on');var go=$('goO');if(go)go.classList.remove('on');var bl=$('blindO');if(bl)bl.classList.remove('on');var ri=$('rdInfo');if(ri)ri.textContent='\u7B2C '+G.rd+' \u56DE\u5408';var mi=$('mli');if(mi)mi.textContent='x'+G.mult+(G.da?' \u51B3\u6597':'');try{window.rdAll()}catch(e){}try{window.enActs()}catch(e){}try{window.upBan()}catch(e){}if(p==='duel_prompt')try{window.upDuel()}catch(e){}if(p==='loading'&&G.bp>=0){G.phase='play';G.rs=true;try{window.rdAll()}catch(e){}try{window.enActs()}catch(e){}try{window.upBan()}catch(e){}}}
function sG(){if(!hG()){T('\u6E38\u620F\u672A\u52A0\u8F7D');return}if(G.phase==='setup'){T('\u8FD8\u6CA1\u5F00\u59CB');return}var s=cS();if(!s){T('\u5B58\u6863\u5931\u8D25');return}try{localStorage.setItem(SK,JSON.stringify(s));var m=JSON.parse(localStorage.getItem(MK)||'[]');m.unshift({t:Date.now(),rd:G.rd,p1:G.p[0]?.n,p2:G.p[1]?.n});if(m.length>10)m.length=10;localStorage.setItem(MK,JSON.stringify(m));T('\u5DF2\u5B58\u6863(\u7B2C'+G.rd+'\u56DE\u5408)')}catch(e){T('\u5B58\u6863\u5931\u8D25')}}
...
// Cheat panel
function rP(){if(!CB||!hG())return;var h0=CB.querySelector('.wch0'),h1=CB.querySelector('.wch1');if(h0)h0.textContent=G.p[0].hp+'/'+G.p[0].mx;if(h1)h1.textContent=G.p[1].hp+'/'+G.p[1].mx;var s0=CB.querySelector('.wcs0'),s1=CB.querySelector('.wcs1');if(s0)s0.value=G.p[0].hp;if(s1)s1.value=G.p[1].hp;var bs=CB.querySelectorAll('.wd-cbtn');for(var i=0;i<bs.length;i++){var b=bs[i];if(b.dataset.ng)b.disabled=!iG()}}
function cHP(i){if(!iG()){T('\u6E38\u620F\u672A\u5F00\u59CB');return}var sl=CB.querySelector(i===0?'.wcs0':'.wcs1');if(!sl)return;var v=parseInt(sl.value);if(isNaN(v))return;G.p[i].hp=Math.min(G.p[i].mx,Math.max(0,v));G.p[i].al=G.p[i].hp>0;try{window.rdAll()}catch(e){}rP();T(G.p[i].n+' HP:'+G.p[i].hp)}
function cRV(){if(!iG()||G.bp<0){T('\u6CA1\u6709\u5B50\u5F39');return}G.br=true;try{window.rdAll()}catch(e){}T('\u5B50\u5F39\u5728\u7B2C'+(G.bp+1)+'\u53D1')}
function cGI(pi,it){if(!iG()){T('\u6E38\u620F\u672A\u5F00\u59CB');return}if(it==='flag'&&!(G.s&&G.s.guildMode)){}G.p[pi].it[it]=1;if(it==='book'){G.p[pi].ba=false;G.p[pi].bu=false}if(it==='flag'){G.p[pi].fa=false;G.p[pi].fc=1;G.p[pi].ft=0}try{window.rdAll()}catch(e){}var ns={watch:'\u6000\u8868',book:'\u4E66',check:'\u67E5\u770B',flag:'\u65D7',wash:'\u6D17\u67AA',medkit:'\u6025\u6551',armor:'\u9632\u5F39',rapid:'\u8FDE\u5C04',freeze:'\u51B0\u51BB',swap:'\u4EA4\u6362'};T(G.p[pi].n+'\u83B7\u5F97'+(ns[it]||it))}
function cGA(pi){var ks=['watch','book','check','flag','wash','medkit','armor','rapid','freeze','swap'];for(var i=0;i<ks.length;i++)cGI(pi,ks[i])}
function cSK(){if(!iG()){T('\u6E38\u620F\u672A\u5F00\u59CB');return}try{G.bf=true;G.phase='ready';setTimeout(function(){if(G.phase!=='go'&&typeof window.nxtRd==='function')window.nxtRd()},100);T('\u8DF3\u8F6C\u4E0B\u4E00\u56DE\u5408')}catch(e){T('\u65E0\u6CD5\u8DF3\u8F6C')}}
function cFH(){if(!iG()){T('\u6E38\u620F\u672A\u5F00\u59CB');return}for(var i=0;i<2;i++){G.p[i].hp=G.p[i].mx;G.p[i].al=true}try{window.rdAll()}catch(e){}rP();T('\u5168\u4F53\u6EE1\u8840')}
function cSM(){if(!iG()){T('\u6E38\u620F\u672A\u5F00\u59CB');return}var sl=CB.querySelector('.wcms');if(!sl)return;G.mult=parseInt(sl.value);var mi=$('mli');if(mi)mi.textContent='x'+G.mult+(G.da?' \u51B3\u6597':'');T('\u500D\u7387 x'+G.mult)}
function cWN(){if(!iG()){T('\u6E38\u620F\u672A\u5F00\u59CB');return}var cp=G.cp,loser=cp===0?1:0;G.p[loser].hp=0;G.p[loser].al=false;try{window.gameOver(cp)}catch(e){T(G.p[cp].n+'\u80DC\u5229')}}
...
function mkP(){if(CP){CP.classList.toggle('on');if(CP.classList.contains('on'))rP();return}
  ...
  // HP
  var s1=aS('\u8840\u91CF');
  for(var i=0;i<2;i++){(function(ii){var r=document.createElement('div');r.className='wd-cr';var lb=document.createElement('label');lb.textContent='P'+(ii+1);var sl=document.createElement('input');sl.type='range';sl.className=ii===0?'wcs0':'wcs1';sl.min=0;sl.max=10;sl.value=5;sl.style.cssText='width:80px;accent-color:#ff6b35';var vl=document.createElement('span');vl.className=ii===0?'wch0':'wch1';vl.style.cssText='min-width:30px;color:#c0a090;font-size:.8em';vl.textContent=hG()?G.p[ii].hp+'/'+G.p[ii].mx:'5/5';r.appendChild(lb);r.appendChild(sl);r.appendChild(vl);r.appendChild(aB(r,'\u8BBE\u7F6E',function(){cHP(ii)}));s1.appendChild(r)})(i)}CB.appendChild(s1);
  // Items
  var s2=aS('\u9053\u5177');
